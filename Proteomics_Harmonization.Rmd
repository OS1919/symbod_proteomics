...
`create_reduced_detailed_plot` is a specialized function for generating a detailed visualization of the logging data associated with the `reduce_genenames` method from the mqhandler package. This plot offers a granular view of the gene name reduction process, highlighting key steps and outcomes.

Parameters

- `logging`: A DataFrame containing detailed logging data returned by the reduce_genenames method of mqhandler.
...
count_proteins_per_fraction <- function(data, meta, fraction_col = "fraction", sample_col = "sample_name") {
  for (frac in unique(meta[[fraction_col]])) {
    frac_samples <- meta[[sample_col]][meta[[fraction_col]] == frac]
    frac_samples <- intersect(frac_samples, names(data))
    ...
    has_value <- rowSums(!is.na(subset) & subset != 0) > 0
    cat(sprintf("Proteins in %s fraction: %d\n", frac, sum(has_value)))
  }
}

# Before harmonization
cat("=== INITIAL PROTEIN COUNTS ===\n")
count_proteins_per_fraction(prot_data, meta_data)

# After harmonization (at the end)
cat("=== FINAL PROTEIN COUNTS AFTER HARMONIZATION ===\n")
count_proteins_per_fraction(orthologs_prot_data, meta_data)
